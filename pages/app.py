...
def validate_session():
    ...
    if 'debug_mode' in st.session_state and st.session_state.debug_mode:
        st.info(f"**Debug - Cookies:** {cookies.keys()}")
    
    if not session_token or not auth_info_str or not timestamp_str:
        if 'debug_mode' in st.session_state and st.session_state.debug_mode:
            st.info(f"**Debug - Validation Failed:** No cookies found")
    ...
    try:
        ...
        if 'debug_mode' in st.session_state and st.session_state.debug_mode:
            st.info(f"**Debug - Cookie Parse Error:** {e}")
    ...
    if datetime.now() - timestamp > timedelta(minutes=30):
        if 'debug_mode' in st.session_state and st.session_state.debug_mode:
            st.info(f"**Debug - Session Expired:** timestamp={timestamp}, now={datetime.now()}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'debug_mode' in st.session_state and st.session_state.debug_mode:
                st.info("**Debug - API Validation Successful**")
            return True
        else:
            if 'debug_mode' in st.session_state and st.session_state.debug_mode:
                st.info(f"**Debug - API Validation Failed:** Status={response.status_code}, Response={response.text[:200]}")
            st.error("Session validation failed. Please log in again.")
            return False
    except Exception as e:
        if 'debug_mode' in st.session_state and st.session_state.debug_mode:
            st.info(f"**Debug - API Validation Error:** {e}")
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Logout", type="secondary", use_container_width=True):
            ...
            cookies['auth_info'] = ''
            cookies['timestamp'] = ''
            cookies.save()  # Save without expire_at
            if 'debug_mode' in st.session_state and st.session_state.debug_mode:
                st.info("**Debug - Cookies Cleared**")
    ...
    if not df.empty:
        display_columns = [
            'alert_title', 'sourceRef', 'alert_created_at', 'resolution_time_hours',
            'alert_received_time', 'processing_start_time',
            'response_time_minutes', 'assigned_to',
        ...
        column_config = {
            ...
            "processing_start_time": st.column_config.DatetimeColumn(
                "Processing Started",
                format="DD/MM/YYYY HH:mm"
            ),
